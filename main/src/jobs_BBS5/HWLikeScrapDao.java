...
public class HWLikeScrapDao implements HWLikeScrapDaoImpl {

	@Override
	public boolean addLikeID(newbbs5HWCodingVO bbs) {
		String sql = " UPDATE newbbs5HWCodingVO SET LIKEID=? "
				+ " WHERE SEQ=? ";
	...
	public boolean addDisLikeID(newbbs5HWCodingVO bbs) {
		String sql = " UPDATE newbbs5HWCodingVO SET DISLIKEID=? "
				+ " WHERE SEQ=? ";
		Connection conn = null;
		PreparedStatement psmt = null;
		...
		try {
			...
			System.out.println("newbbs5HWCodingVO addDisLikeID fail");
			e.printStackTrace();
		} finally {
			DBClose.close(psmt, conn, null);			
		}
	...
	public String getLikeID(int seq) {
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/6 newbbs5HWCodingVO getLikeID Success");
			...
			System.out.println("4/6 newbbs5HWCodingVO getLikeID Success");
	...
	public String getDisLikeID(int seq) {
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/6 newbbs5HWCodingVO getDisLikeID Success");
			...
			rs = psmt.executeQuery();
			System.out.println("3/6 newbbs5HWCodingVO getDisLikeID Success");
	...
	public boolean isitlikeid(int seq, String serchlikeid) {
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/6 newbbs5HWCodingVO getdetail Success");			
		...
		while(rs.next()){		
			...
			dto = new newbbs5HWCodingVO(seq1, id, title, tagname, content, wdate, del, readcount, likecount, likeid,dislikeid, likeidyn);
			
			
		}
		System.out.println("3/6 getdetail Success");		
		} catch (SQLException e) {	
			System.out.println("newbbs5HWCodingVO getdetail fail");		
			e.printStackTrace();
		} finally{
			DBClose.close(psmt, conn, rs);	
		}
	...
	public boolean isitdislikeid(int seq, String serchlikeid) {
		...
		while(rs.next()){		
			...
			dto = new newbbs5HWCodingVO(seq1, id, title, tagname, content, wdate, readcount, likecount, likeid, dislikeid, dislikeidyn);
			
			
		}
		System.out.println("3/6 isitdislikeid Success");		
		} catch (SQLException e) {	
			System.out.println("newbbs5HWCodingVO isitdislikeid fail");		
			e.printStackTrace();
		} finally{
			DBClose.close(psmt, conn, rs);	
		}
	...
	public boolean deleteLikeID(String deleteid, int seq) {
		...
		try {
			conn = DBConnection.getConnection();			
			System.out.println("1/6 newbbs5HWCodingVO deleteLikeID Success");
