...
public class newbbs5HWCodingPDSDao implements newbbs5HWCodingPDSDaoImpl {

	@Override
	public boolean writePds(newbbs5HWCodingPDSVO pds) {
		String sql = " INSERT INTO newbbs5HWCodingPDSVO( "
				+ " SEQ, ID, FILENAME, PARENT, REGDATE) "
				+ " VALUES(SEQ_newbbs5HWCodingPDSVO.NEXTVAL, "
				+ " ?, ?,?, SYSDATE) ";
	...
	@Override
	public newbbs5HWCodingPDSVO getPdsBbs(int parent) {
		String sql = " SELECT SEQ, ID, FILENAME, PARENT, REGDATE "
				+ " FROM SEQ_newbbs5HWCodingPDSVO "
				+ " WHERE PARENT=? ";
		...
		try {
			...
			if(rs.next()) {
				dto = new newbbs5HWCodingPDSVO(rs.getInt(1),		//seq
						rs.getString(2),	//id
						rs.getString(3),	
						rs.getInt(4),
	...
	public boolean pdsupdate(int seq, String filename) {
		String sql = " UPDATE SEQ_newbbs5HWCodingPDSVO SET "
				+ " FILENAME=? "
				+ " WHERE SEQ=? ";
	...
	public List<newbbs5HWCodingPDSVO> getpdsList(int parent) {
		...
		PreparedStatement psmt = null;
		ResultSet rs = null;
		List<newbbs5HWCodingPDSVO> list = new ArrayList<newbbs5HWCodingPDSVO>();
		...
		try {
			...
			while(rs.next()){
				...
				newbbs5HWCodingPDSVO dto = new newbbs5HWCodingPDSVO(rs.getInt(1),
						rs.getString(2),
						rs.getString(3),
						rs.getInt(4),
						rs.getString(5)
